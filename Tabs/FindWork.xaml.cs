...
namespace OrdersApp.Tabs
{
    public partial class FindWork : UserControl
    {
        ...
        public FindWork()
        {
            ...
            InitializeComponent();
            applyButton.Focus();
            ordersDataGrid.ItemsSource = db.Orders.Where(x => x.Status != "Ожидает подтверждения").ToList();
        ...
        private void ordersDataGrid_AutoGeneratedColumns(object? sender, EventArgs e)
        {
            ...
            ordersDataGrid.Columns[1].Header = "оплата";
            ordersDataGrid.Columns[2].Header = "описание";
            ordersDataGrid.Columns[3].Header = "номер";
            ordersDataGrid.Columns[4].Header = "сколько людей требуется";
            ordersDataGrid.Columns[5].Header = "статус";
        ...
        private void SortButton_Click(object sender, RoutedEventArgs e)
        {
            if (radioButtonStatus_NotImportant.IsChecked == false)
            {
                ...
                ordersDataGrid.ItemsSource = db.Orders.Where(x =>
                    ...
                    x.Status == status.ToString()).ToList();
            }
            else
            {
                ordersDataGrid.ItemsSource = db.Orders.Where(x =>
        ...
        private void applyButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                Order order = db.Orders.Where(x => x.ID == ((Order)ordersDataGrid.SelectedItem).ID).FirstOrDefault()!;
                User user = db.Users.Where(x => x.Login == AuthWindow.login).FirstOrDefault()!;
                if (order.Status != "Processing" && order != null) {
                    ...
                    snackBar.MessageQueue?.Enqueue
                        ($"вы приняли заказ {order!.ID}", null, null, null, false, true, TimeSpan.FromSeconds(3));
                    App.logWriter!.WriteLine($"Order number {order!.ID} applied by '{user.Login}' with ID={user.UserId}\t\t\t\t{(DateTime.Now).ToLongTimeString()}");
                }
                else
                {
                    snackBar.MessageQueue?.Enqueue
                        ($"вы уже приняли заказ {order!.ID}", null, null, null, false, true, TimeSpan.FromSeconds(3));
                    App.logWriter!.WriteLine($"Order number {order!.ID} again applied by '{user.Login}' with ID={user.UserId}\t\t\t\t{(DateTime.Now).ToLongTimeString()}");
                }
            }
            catch (Exception)
            {
                snackBar.MessageQueue?.Enqueue
                    ("сначала выберите заказ", null, null, null, false, true, TimeSpan.FromSeconds(3));
            }
        ...
        {
            snackBar.MessageQueue?.Enqueue
                    ("выберите один из заказов и нажмите \"принять заказ\"", null, null, null, false, true, TimeSpan.FromSeconds(3));
        }
