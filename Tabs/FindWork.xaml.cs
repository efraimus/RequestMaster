...
namespace OrdersApp.Tabs
{
    public partial class FindWork : UserControl
    {
        ...
        public FindWork()
        {
            ...
            InitializeComponent();
            applyButton.Focus();
            ordersDataGrid.ItemsSource = db.Orders.Where(x => x.Status != "Awaiting confirmation").ToList();
            ...
            try
            {
                ...
                if (order.Status != "Processing" && order != null) {
                    ...
                    snackBar.MessageQueue?.Enqueue
                        ($"You have applied the order {order!.ID}", null, null, null, false, true, TimeSpan.FromSeconds(3));
                    App.logWriter!.WriteLine($"Order number {order!.ID} applied by '{user.Login}' with ID={user.UserId}\t\t\t\t{(DateTime.Now).ToLongTimeString()}");
                }
                else
                {
                    snackBar.MessageQueue?.Enqueue
                        ($"You have already applied the order {order!.ID}", null, null, null, false, true, TimeSpan.FromSeconds(3));
